...
interface StatusHistoryItem {
  id: string | number;
  a_estado_nombre: string;
  de_estado_nombre?: string | null;
  fecha_cambio: string;
  motivo?: string | null;
}
  ...
  return (
    <div className="relative">
      {statusHistory.map((item, index) => {
        const estadoNombre = item.a_estado_nombre || "Desconocido";
        const config = STATUS_CONFIG[estadoNombre] || {
          ...DEFAULT_CONFIG,
          label: estadoNombre,
        };
            ...
            <div className="flex-1 pt-1">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h4
                    className={`text-lg font-bold ${isFirst ? "text-[#202e5e]" : "text-slate-800"}`}
                  >
                    {config.label}
                  </h4>
                  <span
                    className={`text-sm font-medium ${isFirst ? "text-[#202e5e]" : "text-slate-500"}`}
                  >
