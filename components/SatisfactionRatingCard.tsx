...
export function SatisfactionRatingCard({
  rating,
  comment,
  className,
}: SatisfactionRatingCardProps) {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        ...
        stars.push(
          <span
            key={i}
            className="text-amber-400 text-2xl leading-none relative"
          >
            <span className="absolute inset-0 text-amber-200">★</span>
            <span className="relative inline-block overflow-hidden w-[0.5em]">
              ★
            </span>
          </span>,
        );
        ...
        stars.push(
          <span
            key={i}
            className="text-amber-200 dark:text-amber-800 text-2xl leading-none"
          >
            ★
          </span>,
        );
